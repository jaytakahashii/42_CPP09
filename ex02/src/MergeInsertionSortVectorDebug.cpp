...
void MergeInsertionSortVectorDebug::sort(std::vector<int>& data) {
  if (data.size() <= 1)
    return;
...
// 再帰的に sort
void MergeInsertionSortVectorDebug::recursiveSort(std::vector<int>& data,
...
void MergeInsertionSortVectorDebug::insertPending(
    std::vector<int>& sorted, const std::vector<int>& pend) {
  size_t n = pend.size();
...
// Jacobsthal index を生成
std::vector<size_t> MergeInsertionSortVectorDebug::getJacobsthalIndices(
    size_t size) {
